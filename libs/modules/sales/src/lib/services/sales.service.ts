...
import { createFormData, IResponse, options } from '@neural/shared/data';
...
export class SalesService {
  ...
  getSales(
    ...
    return this.http
      .get<IResponse<IPurchases.IData>>(`${this.url}/v10/sale`, {
        params,
      })
      .pipe(
        delay(500),
  ...
  updateFulfillment(
    ...
    const { ['uuid']: uuid, ...rest } = fulfillment;
    ...
    return this.http
      .put(`${this.url}/v10/sale/fulfillment/${sale.uuid}/${uuid}`, formData, {
        headers,
      })
      ...
      .pipe(
        ...
        map((res: any) => {
          const result = {
            ...sale,
            fulfillments: [...res.response.data.fulfillments],
          };
  ...
  updatePurchaseFulfillment({
  ...
  }): Observable<IPurchases.ISaleFulfillment[]> {
    const { ['uuid']: uuid, ...rest } = fullFillment;
    ...
    return this.http
      .put<IResponse<IPurchases.ISaleFulfillment[]>>(
        `${this.url}/v10/sale/fulfillment/${saleUuid}/${fullFillment.uuid}`,
        formData,
        {
          headers,
        }
      ...
      .pipe(
        delay(500),
        map((res) => res.response.data.fulfillments),
        catchError((error: any) => throwError(error))
      );
  }
  ...
  getSale(uuid: string): Observable<IPurchases.IDocument> {
    return this.http
      .get<IResponse<IPurchases.IDocument>>(`${this.url}/v10/sale/${uuid}`)
  ...
  downloadSaleDocument(
    ...
    return this.http
      .get<IResponse<ISales.IReportData>>(`${this.url}/v10/analytic/report/sale/download`, {
        params,
      })
      .pipe(
        delay(500),
        map((res) => res.response.data.analytics.sales.url),
        catchError((error: any) => throwError(error))
      );
  }
}
