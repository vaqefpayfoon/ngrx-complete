...
export class BrandSeriesExistsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([
      permissionTags.Model.LIST_BRANDS_AND_SERIES,
    ]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Model.LIST_BRANDS_AND_SERIES];
        }
