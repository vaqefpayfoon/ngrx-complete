...
export class BusinessExistsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([permissionTags.Business.GET_BUSINESS]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Business.GET_BUSINESS];
        }
