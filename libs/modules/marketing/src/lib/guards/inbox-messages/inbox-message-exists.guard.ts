...
export class InboxMessageExistsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([permissionTags.Inbox.GET_INBOX_MESSAGE]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Inbox.GET_INBOX_MESSAGE];
        }
