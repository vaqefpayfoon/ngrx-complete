...
export class PromotionExistsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([permissionTags.Promo.GET_PROMO]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Promo.GET_PROMO];
        }
