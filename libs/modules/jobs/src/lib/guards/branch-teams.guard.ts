...
export class BranchTeamsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([permissionTags.Branch.GET_BRANCH_TEAM]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Branch.GET_BRANCH_TEAM];
        }
