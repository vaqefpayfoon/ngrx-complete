...
// interfaces
import { IModuleEventBranch } from '../../models/sms-module.interface';
import {
  BranchNotification,
  EventSmsTitle,
...
@Component({
  selector: 'neural-sms-module-event',
  templateUrl: './sms-module-event.component.html',
  styleUrls: [
    './sms-module-event.component.scss',
    '../branch-form/branch-form.component.scss',
    '../sms-module/sms-module.component.scss',
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SmsModuleEventComponent implements OnChanges, OnInit {
  ...
  private createModuleEventBranch(): FormGroup {
    return this.fb.group({
      ...
      sendSmsDaysBefore: ['', Validators.compose([Validators.required])],
    });
  }
  ...
  addModuleEventBranch(moduleEventBranch?: IModuleEventBranch): void {
    if (moduleEventBranch) {
      const createModuleEventBranch = this.fb.group({
        ...
        body: [
          moduleEventBranch.body,
          Validators.compose([Validators.required]),
        ],
        key: [moduleEventBranch.key, Validators.compose([Validators.required])],
        sendSmsDaysBefore: [moduleEventBranch.sendSmsDaysBefore, Validators.compose([Validators.required])],
      });
  ...
  onChangeTitle(event: MatSelectChange, index: number) {
    ...
    for (const key in EventSmsTitle[this.key.value]) {
      if (
        Object.prototype.hasOwnProperty.call(EventSmsTitle[this.key.value], key) &&
        value === EventSmsTitle[this.key.value][key]
      ) {
  ...
  get eventTitle(): typeof EventSmsTitle {
    return EventSmsTitle;
  }
  ...
  isEventManualBooking(index: number): boolean {
    return this.events.controls[index].get('title').value == EventSmsTitle['manual-reservation']['manual-booking-repair-order']
  }
}
