...
export class FleetsGuard implements CanActivate {
  constructor(
  ...
  canActivate(): Observable<boolean> {
    return this.checkPermission([permissionTags.Fleet.LIST_FLEET]).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  checkPermission(permission: string[]): Observable<boolean> {
    return this.permissionValidatorService.isAvailable(permission).pipe(
      map((key) => {
        if (key) {
          return key[permissionTags.Fleet.LIST_FLEET];
        }
